...
from prompt import (
    PromptController, ORIPromptController
)
...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None, task_embed=None):
        # super(T5Stack, self).__init__(config, embed_tokens, task_embed)
        ...
        if config.encoder_prompt_config:
            self.prompt_modules = ORIPromptController(config.encoder_prompt_config)
        else:
    ...
    def forward(
        ...
        #     prefix_attention_mask = torch.ones(
        #         B, self.config.pre_seq_len, dtype=inputs_embeds.dtype, device=inputs_embeds.device
        #     )
        #     attention_mask = torch.cat([prefix_attention_mask, attention_mask], dim=1)
        ...
        # TODO
        if self.config.use_prompt and self.config.prompt_encoder:
            prefix_attention_mask = torch.zeros(
                extended_attention_mask.size(0),
                extended_attention_mask.size(1),
                extended_attention_mask.size(2),
                self.config.pre_seq_len, dtype=inputs_embeds.dtype, device=inputs_embeds.device
            )
            extended_attention_mask = torch.cat([prefix_attention_mask, extended_attention_mask], dim=3)
        ...
        if self.config.num_layers > 0:
            ...
            if self.config.use_prompt and self.config.prompt_encoder:
                # seq_length = L + V_L
                # q_len = seq_length
                # k_len = seq_length + self.config.pre_seq_len
                # # [1, n_heads, Q_len, K_len]
                # text_position_bias = self.block[0].layer[0].SelfAttention.compute_bias(L, L + self.config.pre_seq_len)
                # num_heads = text_position_bias.size(1)
                # position_bias = text_position_bias.new_zeros(1, num_heads, q_len, k_len)
                # position_bias[:, :, :L, :L + self.config.pre_seq_len] = text_position_bias
                ...
                num_heads = text_position_bias.size(1)
                position_bias = text_position_bias.new_zeros(1, num_heads, q_len, k_len)
                position_bias[:, :, :L, self.config.pre_seq_len:L + self.config.pre_seq_len] = text_position_bias
                position_bias = position_bias + extended_attention_mask
...
class FewVLM(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        if config.decoder_prompt_config:
            self.prompt_modules = ORIPromptController(config.decoder_prompt_config)
        else:
            self.prompt_modules = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.encoder_prompt_config is not None and self.config.encoder_prompt_config.prompt_len > 0:  # todo encoder or decoder
            prefix_attention_mask = torch.ones(
