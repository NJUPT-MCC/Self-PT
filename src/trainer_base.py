...
from my_transformers.modeling_t5 import T5LayerNorm
import modeling_t5
...
from prompt import EncoderPromptConfig, DecoderPromptConfig, PromptController, PromptController_cross, PromptConfig, ORIPromptController
...
class TrainerBase(object):
    ...
    def create_config(self):
        ...
        if args.use_prompt:
            ...
            config.prompt_config.reduction_factor = args.prompt_reduction_factor
            config.prompt_config.phm_rank = args.prompt_phm_rank
            config.prompt_config.factorized_phm = args.prompt_factorized_phm
            config.prompt_config.hypercomplex_division = args.prompt_hypercomplex_division

            config.prompt_input_type = args.prompt_input_type
            config.prompt_cross = args.prompt_cross
    ...
    def print_trainable_params_percentage(self, model):
        # if "t5-base" in self.args.backbone:
    ...
    def unfreeze_parameters(self):
        ...
        for name, sub_module in self.model.named_modules():
            if self.args.decoder_prompt_len > 0 or self.args.encoder_prompt_len > 0 or self.args.use_prompt:
                if isinstance(sub_module, (PromptController, PromptController_cross, ORIPromptController)):
                    print(f"{name} is trainable...")
                    # if len(name.split(".")) < 7: # this will not consider layer norms inside adapters then.
                    for param_name, param in sub_module.named_parameters():
                        param.requires_grad = True
